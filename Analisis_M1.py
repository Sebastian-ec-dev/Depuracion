...
def evaluar_coherencia_multimodal(
    ...
    if (tipo.upper() == "IMAGE"):
        prompt_texto = Prompts.prompt_M1_Imagen()
    ...
    if tipo.upper() == "IMAGE":
        ...
        messages[1]["content"].append({"type": "video", **visual})
    else:
        raise ValueError(f"Tipo no soportado: {tipo}. Usa 'IMAGE' o 'VIDEO'")

    # Agregar el prompt de análisis al final
    messages[1]["content"].append({"type": "text", "text": prompt_original})
...
def analizar_contenido(ruta, tipo="IMAGE", prompt_original = ""):
    try:
        result = evaluar_coherencia_multimodal(ruta, tipo, prompt_original)
        return result
